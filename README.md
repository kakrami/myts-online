## v6.20.99 — conservative identity and minutes reconciliation
...
# v6.20.99 — Provider-enforced database limits
...
6.20.99 corrects the database reservation estimate for atomic single-row match publications. Unbounded operations retain their conservative allowance; daily limits are unchanged.

6.20.99 adds bounded, admin-requested raw goalkeeper tracking diagnostics for established same-season keeper candidates. It does not assign identities or update analytics caches. Explicit diagnostics use the existing API allowance; background limits remain unchanged.

6.20.99 checks all game-roster goalkeeper candidates in bounded diagnostic pages, prioritizing established keepers. Each page compares one half and retains the existing six-request maximum.
